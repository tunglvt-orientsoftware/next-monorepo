...
import { User, UserPlus, UserMinus, Check, X, ArrowLeft } from 'lucide-react'
...
export default function InvitePage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Get target profile by invite code
      ...
      if (profile) {
        ...
        if (user && user.id !== profile.id) {
          ...
          if (friendship) {
            ...
            setIsReceiver(friendship.friend_id === user.id)
          }
        }
      }
  ...
  if (!targetProfile) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#fdfcf8] font-serif space-y-4">
        ...
        <h1 className="text-2xl text-slate-900">Invite Not Found</h1>
        <p className="text-slate-500 font-sans">This invite link is invalid or has expired.</p>
        <Link href="/my-memories">
          <Button variant="outline" className="mt-4 rounded-full font-sans">Go Home</Button>
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#fdfcf8] font-serif py-12 px-4 relative">
      <Link href="/my-memories" className="absolute top-8 left-8 inline-flex items-center text-[#c96442] hover:text-[#b05537] font-sans font-medium transition-colors bg-white/80 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-[#c96442]/20">
      ...
      <div className="max-w-md mx-auto mt-12">
        <div className="bg-white p-10 rounded-3xl border border-slate-200 shadow-xl text-center">
          ...
          {isSelf ? (
            <div className="bg-slate-50 text-slate-600 font-sans p-4 rounded-2xl border border-slate-100">
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 text-amber-700 font-sans p-4 rounded-2xl border border-amber-200">
                {isReceiver ? 'Wants to be your friend' : 'Friend request sent'}
              </div>
              {isReceiver && (
                <div className="flex gap-3">
                  <Button onClick={handleAcceptFriend} disabled={actionLoading} className="w-full rounded-full h-12 font-sans bg-[#c96442] hover:bg-[#b05537]">
                    Accept
                  </Button>
                  <Button onClick={handleDeclineFriend} disabled={actionLoading} variant="outline" className="w-full rounded-full h-12 font-sans border-slate-200 hover:bg-slate-50 text-slate-700">
                    Decline
                  </Button>
                </div>
              )}
